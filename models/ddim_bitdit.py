...
class BitDit(nn.Module):
    def __init__(self,
        ...
        codebook_tensor = torch.load("label_codebook.pt") 
        target_radius = 2.0
        scaled_codebook = F.normalize(codebook_tensor, p=2, dim=-1) * target_radius
        self.register_buffer("label_codebook", scaled_codebook)
    ...
    # HyperSpherical GNN Decoding #
    def decode_labels(self, pred_embeddings):
        # 1. Prediction magnitude is ignored; only direction matters
        # This prevents the 'Accuracy Crash' caused by magnitude drift
        norm_pred = F.normalize(pred_embeddings, p=2, dim=-1)
        
        # 2. Compute similarity to the fixed GNN anchors
        # We use the raw unit-normalized codebook here
        similarities = torch.matmul(norm_pred, F.normalize(self.label_codebook, p=2, dim=-1).t())
        
        # 3. Return the index of the closest angular neighbor
    ...
    def forward(self, input_ids: torch.tensor, attention_mask: torch.tensor, seq_labels: torch.tensor,
        ...
        if self.training:
            ...
            if self.loss_type == 'l2':
                ...
                cos_sim = F.cosine_similarity(pred[targets_mask.bool()], targets[targets_mask.bool()], dim=-1)
                loss = (1 - cos_sim).mean()
            else:
